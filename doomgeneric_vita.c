...
static void init_display(void) {
    ...
    if (fb_memuid < 0) { debug_logf("Alloc failed: 0x%08X", fb_memuid); return; }
    sceKernelGetMemBlockBase(fb_memuid, &fb_base);
    memset(fb_base, 0, sz);
    memset(&fb, 0, sizeof(fb));
    fb.size = sizeof(fb); fb.base = fb_base; fb.pitch = 960;
    ...
    sceDisplayWaitVblankStart(); display_ready = 1;
}
...
static void kq_push(int p, unsigned char k) {
    ...
    if (n == kq_r) return;
    kq[kq_w].pressed = p; kq[kq_w].key = k; kq_w = n;
}
static void analog_axis(int val, int nk, int pk, int *nh, int *ph) {
    int wn = val < -DEADZONE, wp = val > DEADZONE;
    if (wn && !*nh) { kq_push(1, nk); *nh = 1; }
    if (!wn && *nh) { kq_push(0, nk); *nh = 0; }
    if (wp && !*ph) { kq_push(1, pk); *ph = 1; }
    if (!wp && *ph) { kq_push(0, pk); *ph = 0; }
}
...
static void do_poll_input(void) {
    ...
    { struct { unsigned btn; unsigned char key; } bm[] = {
        ...
        {SCE_CTRL_RTRIGGER, KEY_FIRE}, {SCE_CTRL_LTRIGGER, KEY_RSHIFT},
        {SCE_CTRL_START, KEY_ESCAPE}, {SCE_CTRL_SELECT, KEY_ENTER}, {0,0}
      };
      for (i = 0; bm[i].btn; i++) {
...
/* SFX ENGINE */
typedef struct {
    const byte *data; int length, pos_fixed, step_fixed;
...
typedef struct { int lumpnum; const byte *samples; int length, samplerate; } sfx_cache_entry_t;
...
static sfx_cache_entry_t *sfx_cache_get(int lumpnum) {
    ...
    rawlen = W_LumpLength(lumpnum); if (rawlen < 8) return NULL;
    raw = W_CacheLumpNum(lumpnum, PU_STATIC); if (!raw) return NULL;
    format = raw[0] | (raw[1] << 8); rate = raw[2] | (raw[3] << 8);
    ...
    { const byte *pcm = raw + 8; int pcm_len = nsamples;
      if (pcm_len > 32) { pcm += 16; pcm_len -= 32; }
      if (sfx_cache_count >= SFX_CACHE_MAX) return NULL;
      i = sfx_cache_count++;
      sfx_cache[i].lumpnum = lumpnum; sfx_cache[i].samples = pcm;
      sfx_cache[i].length = pcm_len; sfx_cache[i].samplerate = rate; }
    return &sfx_cache[i];
}
...
#pragma pack(push, 1)
typedef struct { uint8_t tremolo, attack, sustain, waveform, scale, level; } genmidi_op_t;
typedef struct { genmidi_op_t modulator; uint8_t feedback; genmidi_op_t carrier; uint8_t unused; int16_t base_note_offset; } genmidi_voice_t;
typedef struct { uint16_t flags; uint8_t fine_tuning, fixed_note; genmidi_voice_t voices[2]; } genmidi_instr_t;
#pragma pack(pop)

static const uint8_t opl_mod_offset[9]={0x00,0x01,0x02,0x08,0x09,0x0A,0x10,0x11,0x12};
static const uint8_t opl_car_offset[9]={0x03,0x04,0x05,0x0B,0x0C,0x0D,0x13,0x14,0x15};
static const uint16_t opl_freq_table[12]={0x157,0x16B,0x181,0x198,0x1B0,0x1CA,0x1E5,0x202,0x220,0x241,0x263,0x287};

typedef struct { int active, mus_channel, note, volume; uint32_t age; } opl_voice_t;
typedef struct { int volume, patch, pitch_bend; } opl_mus_chan_t;
typedef struct {
    opl3_chip chip; genmidi_instr_t *genmidi; int genmidi_loaded;
    opl_voice_t voices[OPL_NUM_VOICES]; opl_mus_chan_t channels[16]; uint32_t voice_age;
    const byte *mus_data; int mus_len, mus_pos, score_start, score_len;
...
static void opl_write(uint16_t reg, uint8_t val) { OPL3_WriteReg(&opl_music.chip, reg, val); }
...
static void load_genmidi(void) {
    ...
    len = W_LumpLength(lump); data = W_CacheLumpNum(lump, PU_STATIC);
...
static void opl_write_operator(int slot, genmidi_op_t *op, int vol) {
    ...
    if (vol >= 0) {
        l = 0x3F - (op->level & 0x3F); l = (l * vol) / 127;
        fl = 0x3F - l; if (fl < 0) fl = 0; if (fl > 0x3F) fl = 0x3F;
        opl_write(0x40 + slot, (op->scale & 0xC0) | fl);
    } else { opl_write(0x40 + slot, (op->scale & 0xC0) | (op->level & 0x3F)); }
...
static void opl_set_instrument(int voice, genmidi_voice_t *gv, int volume) {
    ...
    opl_write_operator(opl_mod_offset[voice], &gv->modulator, is_add ? volume : -1);
...
static void opl_update_volume(int voice, int volume, genmidi_voice_t *gv) {
    int l, fl;
    l = 0x3F - (gv->carrier.level & 0x3F); l = (l * volume) / 127;
    fl = 0x3F - l; if (fl < 0) fl = 0; if (fl > 0x3F) fl = 0x3F;
    opl_write(0x40 + opl_car_offset[voice], (gv->carrier.scale & 0xC0) | fl);
    if (gv->feedback & 0x01) {
        l = 0x3F - (gv->modulator.level & 0x3F); l = (l * volume) / 127;
        fl = 0x3F - l; if (fl < 0) fl = 0; if (fl > 0x3F) fl = 0x3F;
...
static void opl_key_on(int voice, int note) {
    int oct, fn; uint16_t freq;
    if (note < 0) note = 0; if (note > 127) note = 127;
    oct = (note / 12) - 1; fn = note % 12;
...
static void opl_key_off(int v) { opl_reg_b0[v] &= ~0x20; opl_write(0xB0+v, opl_reg_b0[v]); }
static void opl_silence_voice(int v) { opl_reg_b0[v]=0; opl_write(0xB0+v,0); opl_write(0xA0+v,0); }

static int opl_alloc_voice(int ch, int pri) {
    int i, best; uint32_t oldest; (void)pri;
    for (i=0;i<OPL_NUM_VOICES;i++) if (!opl_music.voices[i].active) return i;
    best=0; oldest=0xFFFFFFFF;
    for (i=0;i<OPL_NUM_VOICES;i++) if (opl_music.voices[i].age<oldest){oldest=opl_music.voices[i].age;best=i;}
    opl_key_off(best); opl_music.voices[best].active=0; return best;
}

static genmidi_voice_t *get_voice_instr(int vi) {
    int ch,patch;
    if (!opl_music.genmidi_loaded) return NULL;
    ch=opl_music.voices[vi].mus_channel; patch=opl_music.channels[ch].patch;
    if (ch==PERCUSSION_CHAN){int n=opl_music.voices[vi].note;if(n>=35&&n<=81)patch=128+n-35;else return NULL;}
...
static void mus_opl_note_on(int channel,int note,int volume) {
    int voice,patch,mn; genmidi_instr_t *inst; genmidi_voice_t *gv;
    if (!opl_music.genmidi_loaded) return;
    patch=opl_music.channels[channel].patch;
    if (channel==PERCUSSION_CHAN){if(note<35||note>81)return;patch=128+note-35;}
    ...
    if (inst->flags&GENMIDI_FLAG_FIXED) mn=inst->fixed_note;
    else{mn=note;int off=(int)(int16_t)gv->base_note_offset;if(off>-48&&off<48)mn+=off;}
    if(mn<0)mn=0;if(mn>127)mn=127;
    if(volume<0)volume=opl_music.channels[channel].volume;if(volume>127)volume=127;
    opl_set_instrument(voice,gv,volume);opl_key_on(voice,mn);
    opl_music.voices[voice].active=1;opl_music.voices[voice].mus_channel=channel;
    opl_music.voices[voice].note=note;opl_music.voices[voice].volume=volume;
    opl_music.voices[voice].age=opl_music.voice_age++;
}

static void mus_opl_note_off(int ch,int note){int i;for(i=0;i<OPL_NUM_VOICES;i++)if(opl_music.voices[i].active&&opl_music.voices[i].mus_channel==ch&&opl_music.voices[i].note==note){opl_key_off(i);opl_music.voices[i].active=0;}}
static void mus_opl_all_off(int ch){int i;for(i=0;i<OPL_NUM_VOICES;i++)if(opl_music.voices[i].mus_channel==ch&&opl_music.voices[i].active){opl_key_off(i);opl_music.voices[i].active=0;}}
static byte mus_rb(void){if(opl_music.mus_pos>=opl_music.mus_len)return 0;return opl_music.mus_data[opl_music.mus_pos++];}
...
static void mus_process_event(void) {
    ...
    if (opl_music.mus_pos>=opl_music.score_start+opl_music.score_len) {
        if(opl_music.looping){opl_music.mus_pos=opl_music.score_start;for(i=0;i<OPL_NUM_VOICES;i++){opl_silence_voice(i);opl_music.voices[i].active=0;}}
        else opl_music.playing=0; return;
    }
    ev=mus_rb();channel=ev&0x0F;type=(ev>>4)&0x07;last=ev&0x80;
    switch(type){
    case 0:{byte n=mus_rb();mus_opl_note_off(channel,n&0x7F);break;}
    case 1:{byte nb=mus_rb();int n=nb&0x7F,v=-1;if(nb&0x80){v=mus_rb()&0x7F;opl_music.channels[channel].volume=v;}mus_opl_note_on(channel,n,v);break;}
    case 2:{byte pb=mus_rb();opl_music.channels[channel].pitch_bend=pb;break;}
    case 3:{byte sys=mus_rb();if(sys==10||sys==11||sys==14)mus_opl_all_off(channel);break;}
    case 4:{byte ctrl=mus_rb(),val=mus_rb();if(ctrl==0)opl_music.channels[channel].patch=val;
        else if(ctrl==3){opl_music.channels[channel].volume=val&0x7F;
            for(i=0;i<OPL_NUM_VOICES;i++){if(opl_music.voices[i].active&&opl_music.voices[i].mus_channel==channel){
                genmidi_voice_t*gv=get_voice_instr(i);if(gv){int cv=(opl_music.voices[i].volume*(val&0x7F))/127;if(cv>127)cv=127;opl_update_volume(i,cv,gv);}}}}break;}
    case 5:case 6:if(opl_music.looping){opl_music.mus_pos=opl_music.score_start;for(i=0;i<OPL_NUM_VOICES;i++){opl_silence_voice(i);opl_music.voices[i].active=0;}}else opl_music.playing=0;return;
    default:break;}
    if(last){int delay=0;byte db;do{db=mus_rb();delay=(delay<<7)|(db&0x7F);}while(db&0x80);opl_music.delay_left=delay;}
}

static void mus_opl_tick(void){if(!opl_music.playing)return;while(opl_music.delay_left<=0&&opl_music.playing)mus_process_event();if(opl_music.delay_left>0)opl_music.delay_left--;}

static void opl_mix_into(int32_t *accum,int ns) {
    int s,mv;if(!opl_music.playing)return;mv=opl_music.music_volume;if(mv<=0)return;
    for(s=0;s<ns;s++){int16_t buf[4];opl_music.tick_counter--;
        if(opl_music.tick_counter<=0){opl_music.tick_counter=opl_music.tick_samples;mus_opl_tick();}
        memset(buf,0,sizeof(buf));OPL3_GenerateResampled(&opl_music.chip,buf);
        accum[s*2+0]+=((int32_t)buf[0]*mv*2)/15;accum[s*2+1]+=((int32_t)buf[1]*mv*2)/15;}
}
...
static void mix_into(int16_t *out, int nsamples) {
    ...
    mvol = sfx_master_vol; if (mvol < 0) mvol = 0; if (mvol > 15) mvol = 15;
...
static void start_audio_system(void) {
    ...
    sceAudioOutSetVolume(sfx_port, SCE_AUDIO_VOLUME_FLAG_L_CH | SCE_AUDIO_VOLUME_FLAG_R_CH, vols);
    sfx_running = 1;
    sfx_thread_id = sceKernelCreateThread("doom_audio", sfx_thread_func, 0x10000100, 0x10000, 0, 0, NULL);
    if (sfx_thread_id < 0) {
        sfx_running = 0; sceAudioOutReleasePort(sfx_port); sfx_port = -1;
        sceKernelDeleteMutex(sfx_mutex); sfx_mutex = -1;
        if (mus_mutex >= 0) { sceKernelDeleteMutex(mus_mutex); mus_mutex = -1; }
        return;
    }
    ...
    if (ret < 0) {
        sfx_running = 0; sceKernelDeleteThread(sfx_thread_id); sfx_thread_id = -1;
        sceAudioOutReleasePort(sfx_port); sfx_port = -1;
        sceKernelDeleteMutex(sfx_mutex); sfx_mutex = -1;
        if (mus_mutex >= 0) { sceKernelDeleteMutex(mus_mutex); mus_mutex = -1; }
        return;
    }
    audio_ready = 1; debug_log("Audio OK");
}
...
void I_Quit(void) {
    sfx_running = 0;
    if (sfx_thread_id >= 0) { sceKernelWaitThreadEnd(sfx_thread_id, NULL, NULL); sceKernelDeleteThread(sfx_thread_id); }
...
void I_Error(const char *error, ...) {
    ...
    debug_log(buf); sfx_running = 0; sceKernelExitProcess(0);
}
void I_WaitVBL(int c) { sceKernelDelayThread(c * 14286); }
int I_GetTime(void) { uint32_t ms = get_ms() - base_time; return (int)(ms * TICRATE / 1000); }
void I_Sleep(int ms) { sceKernelDelayThread(ms * 1000); }
byte *I_ZoneBase(int *size) {
    byte *p; *size = 16 * 1024 * 1024;
    p = (byte *)malloc(*size);
...
boolean I_GetMemoryValue(unsigned int o, void *v, int s) { (void)o; (void)v; (void)s; return 0; }
...
void I_InitGraphics(void) {
    int i; I_VideoBuffer = (byte *)calloc(SCREENWIDTH * SCREENHEIGHT, 1);
    for (i = 0; i < 256; i++) cmap[i] = 0xFF000000u | ((uint32_t)i << 16) | ((uint32_t)i << 8) | i;
}
...
void I_FinishUpdate(void) {
    uint32_t *dst; int x, y, step_x, step_y, sy_f;
    if (!display_ready || !I_VideoBuffer || !fb_base) return;
    dst = (uint32_t *)fb_base;
    step_x = (SCREENWIDTH << 16) / VITA_W; step_y = (SCREENHEIGHT << 16) / VITA_H;
    sy_f = 0;
    for (y = 0; y < VITA_H; y++) {
        int sy = sy_f >> 16; uint32_t *dr; byte *sr; int sx_f;
        if (sy >= SCREENHEIGHT) sy = SCREENHEIGHT - 1;
        dr = dst + y * 960; sr = I_VideoBuffer + sy * SCREENWIDTH; sx_f = 0;
        for (x = 0; x < VITA_W; x++) {
            int sx = sx_f >> 16; if (sx >= SCREENWIDTH) sx = SCREENWIDTH - 1;
            dr[x] = cmap[sr[sx]]; sx_f += step_x;
        }
        sy_f += step_y;
    }
    { SceDisplayFrameBuf dfb; memset(&dfb, 0, sizeof(dfb));
      dfb.size = sizeof(dfb); dfb.base = fb_base; dfb.pitch = 960;
      dfb.pixelformat = SCE_DISPLAY_PIXELFORMAT_A8B8G8R8; dfb.width = 960; dfb.height = 544;
      sceDisplaySetFrameBuf(&dfb, SCE_DISPLAY_SETBUF_NEXTFRAME); }
    sceDisplayWaitVblankStart(); frame_count++;
}
void I_ShutdownGraphics(void) {}
void I_StartFrame(void) {}
void I_StartTic(void) {
    event_t event; do_poll_input();
    while (kq_r != kq_w) {
        event.type = kq[kq_r].pressed ? ev_keydown : ev_keyup;
        event.data1 = kq[kq_r].key; event.data2 = kq[kq_r].key; event.data3 = 0;
        D_PostEvent(&event); kq_r = (kq_r + 1) % KQUEUE_SZ;
    }
}
void I_UpdateNoBlit(void) {}
void I_ReadScreen(byte *scr) { if (I_VideoBuffer) memcpy(scr, I_VideoBuffer, SCREENWIDTH * SCREENHEIGHT); }
...
void I_PrecacheSounds(sfxinfo_t *sounds, int num_sounds) {
    int i, l; for (i = 0; i < num_sounds; i++) { l = I_GetSfxLumpNum(&sounds[i]); if (l >= 0) sfx_cache_get(l); }
}
int I_StartSound(sfxinfo_t *sfx, int channel, int vol, int sep) {
    int lumpnum, best, i, oldest, handle; sfx_cache_entry_t *entry; mix_channel_t *c;
    (void)channel;
    if (!audio_ready || !sfx) return 0;
    lumpnum = sfx->lumpnum;
    if (lumpnum < 0) { lumpnum = I_GetSfxLumpNum(sfx); if (lumpnum < 0) return 0; }
    entry = sfx_cache_get(lumpnum); if (!entry || entry->length <= 0) return 0;
    sceKernelLockMutex(sfx_mutex, 1, NULL);
...
found:
    c = &mix_ch[best]; c->data = entry->samples; c->length = entry->length;
    ...
    { int vl, vr; if (sep < 0 || sep > 255) sep = 128;
    ...
    handle = next_handle++; if (next_handle > 0x7FFFFF00) next_handle = 1;
    c->handle = handle; c->active = 1;
    sceKernelUnlockMutex(sfx_mutex, 1);
    return handle;
}
void I_StopSound(int handle) {
    int i; if (sfx_mutex < 0) return;
    sceKernelLockMutex(sfx_mutex, 1, NULL);
    for (i = 0; i < MIX_CHANNELS; i++) if (mix_ch[i].active && mix_ch[i].handle == handle) { mix_ch[i].active = 0; break; }
    sceKernelUnlockMutex(sfx_mutex, 1);
}
boolean I_SoundIsPlaying(int handle) {
    int i; boolean r = false; if (sfx_mutex < 0) return false;
    sceKernelLockMutex(sfx_mutex, 1, NULL);
    for (i = 0; i < MIX_CHANNELS; i++) if (mix_ch[i].active && mix_ch[i].handle == handle) { r = true; break; }
    sceKernelUnlockMutex(sfx_mutex, 1); return r;
}
...
void I_UpdateSoundParams(int handle, int vol, int sep) {
    ...
    for (i = 0; i < MIX_CHANNELS; i++) {
        if (mix_ch[i].active && mix_ch[i].handle == handle) {
            ...
            if (vl < 0) vl = 0; if (vl > 256) vl = 256;
            if (vr < 0) vr = 0; if (vr > 256) vr = 256;
            mix_ch[i].vol_left = (vl * vol) / 127; mix_ch[i].vol_right = (vr * vol) / 127;
            if (mix_ch[i].vol_left > 256) mix_ch[i].vol_left = 256;
            if (mix_ch[i].vol_right > 256) mix_ch[i].vol_right = 256; break;
        }
    }
    sceKernelUnlockMutex(sfx_mutex, 1);
}
void I_InitSound(boolean use_sfx_prefix) { (void)use_sfx_prefix; start_audio_system(); }
void I_ShutdownSound(void) {
    sfx_running = 0;
    if (sfx_thread_id >= 0) { sceKernelWaitThreadEnd(sfx_thread_id, NULL, NULL); sceKernelDeleteThread(sfx_thread_id); sfx_thread_id = -1; }
...
void I_ShutdownMusic(void) {
    int i; if (mus_mutex >= 0) { sceKernelLockMutex(mus_mutex, 1, NULL);
        opl_music.playing = 0; for (i = 0; i < OPL_NUM_VOICES; i++) { opl_silence_voice(i); opl_music.voices[i].active = 0; }
        sceKernelUnlockMutex(mus_mutex, 1); }
}
void I_SetMusicVolume(int v) {
    if (mus_mutex >= 0) { sceKernelLockMutex(mus_mutex, 1, NULL);
        opl_music.music_volume = v; if (opl_music.music_volume < 0) opl_music.music_volume = 0;
        if (opl_music.music_volume > 15) opl_music.music_volume = 15;
        sceKernelUnlockMutex(mus_mutex, 1); }
}
void I_PauseSong(void) { if (mus_mutex >= 0) { sceKernelLockMutex(mus_mutex, 1, NULL); opl_music.playing = 0; sceKernelUnlockMutex(mus_mutex, 1); } }
void I_ResumeSong(void) { if (mus_mutex >= 0) { sceKernelLockMutex(mus_mutex, 1, NULL); if (opl_music.mus_data) opl_music.playing = 1; sceKernelUnlockMutex(mus_mutex, 1); } }
...
boolean I_MusicIsPlaying(void) {
    boolean r; if (mus_mutex >= 0) { sceKernelLockMutex(mus_mutex, 1, NULL);
        r = opl_music.playing ? true : false; sceKernelUnlockMutex(mus_mutex, 1);
    } else r = opl_music.playing ? true : false; return r;
}
void *I_RegisterSong(void *data, int len) {
    byte *d = (byte *)data, *md; int so, sl, i;
    if (!data || len < 16) return NULL;
    if (d[0] != 'M' || d[1] != 'U' || d[2] != 'S' || d[3] != 0x1A) return (void *)1;
    sl = d[4] | (d[5] << 8); so = d[6] | (d[7] << 8);
    if (so >= len || so < 12) return (void *)1;
    if (sl <= 0 || so + sl > len) sl = len - so;
    md = (byte *)malloc(len); if (!md) return (void *)1;
    memcpy(md, data, len);
    if (mus_mutex >= 0) sceKernelLockMutex(mus_mutex, 1, NULL);
    if (opl_music.mus_data) { free((void *)opl_music.mus_data); opl_music.mus_data = NULL; }
    opl_music.playing = 0;
    for (i = 0; i < OPL_NUM_VOICES; i++) { opl_silence_voice(i); opl_music.voices[i].active = 0; }
    opl_music.mus_data = md; opl_music.mus_len = len;
    opl_music.score_start = so; opl_music.score_len = sl;
    opl_music.mus_pos = so; opl_music.delay_left = 0;
    opl_music.tick_counter = opl_music.tick_samples; opl_music.voice_age = 0;
    for (i = 0; i < 16; i++) { opl_music.channels[i].volume = 100; opl_music.channels[i].patch = 0; opl_music.channels[i].pitch_bend = 64; }
    if (mus_mutex >= 0) sceKernelUnlockMutex(mus_mutex, 1);
    return (void *)md;
}
void I_UnRegisterSong(void *handle) {
    int i; if (!handle || handle == (void *)1) return;
    if (mus_mutex >= 0) sceKernelLockMutex(mus_mutex, 1, NULL);
    opl_music.playing = 0;
    for (i = 0; i < OPL_NUM_VOICES; i++) { opl_silence_voice(i); opl_music.voices[i].active = 0; }
    if (opl_music.mus_data == (const byte *)handle) { opl_music.mus_data = NULL; opl_music.mus_len = 0; }
    if (mus_mutex >= 0) sceKernelUnlockMutex(mus_mutex, 1);
    free(handle);
}
void I_PlaySong(void *handle, boolean looping) {
    ...
    if (opl_music.mus_data == (const byte *)handle) {
        opl_music.mus_pos = opl_music.score_start; opl_music.delay_left = 0;
        opl_music.looping = looping ? 1 : 0; opl_music.tick_counter = opl_music.tick_samples;
        for (i = 0; i < OPL_NUM_VOICES; i++) { opl_silence_voice(i); opl_music.voices[i].active = 0; }
        opl_music.playing = 1;
    }
    if (mus_mutex >= 0) sceKernelUnlockMutex(mus_mutex, 1);
...
int main(int argc, char **argv) {
    int i; const char *iwad = NULL; const char *pwad = NULL;
    ...
    init_display();
    if (!display_ready) { debug_log("FATAL: no display"); sceKernelExitProcess(0); return 1; }
    base_time = get_ms();

    for (i = 0; iwad_paths[i]; i++) {
        SceUID fd = sceIoOpen(iwad_paths[i], SCE_O_RDONLY, 0);
        if (fd >= 0) { sceIoClose(fd); iwad = iwad_paths[i]; break; }
    }
    for (i = 0; pwad_paths[i]; i++) {
        SceUID fd = sceIoOpen(pwad_paths[i], SCE_O_RDONLY, 0);
        if (fd >= 0) { sceIoClose(fd); pwad = pwad_paths[i]; break; }
    }

    if (!iwad) { debug_log("NO doom2.wad!"); sceKernelDelayThread(5000000); sceKernelExitProcess(0); return 1; }
    debug_logf("IWAD: %s", iwad);
    if (pwad) debug_logf("PWAD: %s", pwad);

    base_time = get_ms();

    if (pwad) {
        char *nargv[] = { "BatmanDoom", "-iwad", (char *)iwad, "-file", (char *)pwad, NULL };
